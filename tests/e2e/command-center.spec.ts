...
test.describe('Command Center', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate('document.getElementById("command-center").scrollIntoView({block:"start"})')
    await page.waitForTimeout(3000)
  })
  ...
  test('POS kartica prikazuje promet', async ({ page }) => {
    const cc = page.locator('#command-center')
    // Use first to avoid strict mode (multiple € amounts)
    await expect(cc.getByText(/€[\d,]+/).first()).toBeVisible()
    await expect(cc.getByText('naročil').first()).toBeVisible()
  })

  test('KDS kartica prikazuje statuse', async ({ page }) => {
    const cc = page.locator('#command-center')
    // Use first to avoid strict mode (NOVA appears in multiple places)
    await expect(cc.getByText('NOVA').first()).toBeVisible()
    await expect(cc.getByText('PRIPRAVA').first()).toBeVisible()
    await expect(cc.getByText('PRIPRAVL').first()).toBeVisible()
  })
