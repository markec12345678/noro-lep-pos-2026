...
test.describe('AI Prediction', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate('document.getElementById("ai-prediction").scrollIntoView({block:"start"})')
    await page.waitForTimeout(3000)
  })
  ...
  test('prikaže predikcije grid', async ({ page }) => {
    const sec = page.locator('#ai-prediction')
    // Wait for API to load predictions
    await page.waitForTimeout(2000)
    await expect(sec.getByText('Predikcija/teden').first()).toBeVisible({ timeout: 10000 })
    await expect(sec.getByText('Zmanjka v').first()).toBeVisible()
    await expect(sec.getByText('Zaupanje').first()).toBeVisible()
  })
  ...
  test('toggle med Predikcije in Dobavnica', async ({ page }) => {
    ...
    await expect(sec.getByText('AI samodejna dobavnica')).toBeVisible({ timeout: 5000 })
    await expect(sec.getByRole('button', { name: /Ustvari dobavnico/i })).toBeVisible()

    await sec.getByRole('button', { name: /Predikcije/i }).click()
    await page.waitForTimeout(1000)
    await expect(sec.getByText('Predikcija/teden').first()).toBeVisible({ timeout: 5000 })
  })
