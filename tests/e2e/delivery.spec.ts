...
test.describe('Delivery', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate('document.getElementById("dostava").scrollIntoView({block:"start"})')
    await page.waitForTimeout(3000)
  })
  ...
  test('order feed vsebuje naročila', async ({ page }) => {
    const sec = page.locator('#dostava')
    // Wait for orders to load from API
    await page.waitForTimeout(2000)
    await expect(sec.getByText(/DEL-/).first()).toBeVisible({ timeout: 10000 })
  })
  ...
  test('simuliraj naročilo gumb deluje', async ({ page }) => {
    ...
    await btn.click()
    await page.waitForTimeout(2000)
    await expect(sec.getByText(/DEL-/).first()).toBeVisible({ timeout: 5000 })
  })
